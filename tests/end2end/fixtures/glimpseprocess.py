...
"""Fixtures to run glimpsebrowser in a QProcess and communicate."""
...
from glimpsebrowser.utils import log, utils, javascript, qtutils
...
def is_ignored_lowlevel_message(message):
    ...
    ignored_messages = [
        ...
        # https://ci.appveyor.com/project/glimpsebrowser/glimpsebrowser/build/master-2089/job/or4tbct1oeqsfhfm
        ...
        # /home/travis/build/glimpsebrowser/glimpsebrowser/.tox/py36-pyqt511-cov/lib/
        # python3.6/site-packages/PyQt5/Qt/libexec/QtWebEngineProcess:
        # /lib/x86_64-linux-gnu/libdbus-1.so.3: no version information
        # available (required by /home/travis/build/glimpsebrowser/glimpsebrowser/
...
def is_ignored_chromium_message(line):
    ...
    ignored_messages = [
        ...
        # Local\Temp\1\glimpsebrowser-basedir-1l3jmxq4\data\webengine\
...
class LogLine(testprocess.Line):
    ...
    """A parsed line from the glimpsebrowser log output.
    ...
    def formatted_str(self, colorized=True):
        ...
        This returns a line like glimpse without --json-logging would produce.
...
class GlimpseProc(testprocess.Process):

    """A running glimpsebrowser process used for tests.
    ...
    Attributes:
        ...
        _instance_id: A unique ID for this GlimpseProc instance
    ...
    def _process_line(self, log_line):
        ...
        start_okay_message_load = (
            "load status for <glimpsebrowser.browser.* tab_id=0 "
            "url='about:blank'>: LoadStatus.success")
        start_okay_messages_focus = [
            ## QtWebKit
            "Focus object changed: "
            "<glimpsebrowser.browser.* tab_id=0 url='about:blank'>",
            # when calling QApplication::sync
            "Focus object changed: "
            "<glimpsebrowser.browser.webkit.webview.WebView tab_id=0 url=''>",
            ...
            # Qt >= 5.11
            "Focus object changed: "
            "<glimpsebrowser.browser.webengine.webview.WebEngineView object "
            "at *>",
    ...
    def _executable_args(self):
        profile = self.request.config.getoption('--glimpse-profile-subprocs')
    ...
    def path_to_url(self, path, *, port=None, https=False):
        ...
        URLs like about:... and glimpse:... are handled specially and returned
        verbatim.
        """
        special_schemes = ['about:', 'glimpse:', 'chrome:', 'view-source:',
                           'data:', 'http:', 'https:']
    ...
    def _init_settings(self):
        """Adjust some glimpsebrowser settings after starting."""
        settings = [
            ('messages.timeout', '0'),
    ...
    def send_ipc(self, commands, target_arg=''):
        ...
        delay = self.request.config.getoption('--glimpse-delay')
    ...
    def send_cmd(self, command, count=None, invalid=False, *, escape=True):
        """Send a command to the running glimpsebrowser instance.
    ...
    def get_setting(self, opt, pattern=None):
        """Get the value of a glimpsebrowser setting."""
        if pattern is None:
            cmd = ':set {}?'.format(opt)
        else:
    ...
    def open_path(self, path, *, new_tab=False, new_bg_tab=False,
        ...
        """Open the given path on the local webserver in glimpsebrowser."""
    ...
    def open_url(self, url, *, new_tab=False, new_bg_tab=False,
        ...
        """Open the given url in glimpsebrowser."""
    ...
    def click_element_by_text(self, text):
        ...
        script = (
            ...
            'if (_es.snapshotLength == 0) {{ console.log("glimpse:no elems"); }} '
            'else if (_es.snapshotLength > 1) {{ console.log("glimpse:ambiguous '
            'elems") }} '
            'else {{ console.log("glimpse:okay"); _es.snapshotItem(0).click() }}'
        ).format(text=javascript.string_escape(_xpath_escape(text)))
        self.send_cmd(':jseval ' + script, escape=False)
        message = self.wait_for_js('glimpse:*').message
        if message.endswith('glimpse:no elems'):
            raise ValueError('No element with {!r} found'.format(text))
        if message.endswith('glimpse:ambiguous elems'):
            raise ValueError('Element with {!r} is not unique'.format(text))
        if not message.endswith('glimpse:okay'):
            raise ValueError('Invalid response from glimpsebrowser: {}'
                             .format(message))
...
def glimpseproc_process(qapp, server, request):
    """Fixture for glimpsebrowser process which is started once per file."""
    # Passing request so it has an initial config
    proc = GlimpseProc(request)
    proc.start()
    yield proc
    proc.terminate()
...
@pytest.fixture
def glimpseproc(glimpseproc_process, server, request):
    """Per-test glimpsebrowser fixture which uses the per-file process."""
    request.node._glimpseproc_log = glimpseproc_process.captured_log
    glimpseproc_process.before_test()
    glimpseproc_process.request = request
    yield glimpseproc_process
    glimpseproc_process.after_test()


@pytest.fixture
def glimpseproc_new(qapp, server, request):
    """Per-test glimpsebrowser process to test invocations."""
    proc = GlimpseProc(request)
    request.node._glimpseproc_log = proc.captured_log
