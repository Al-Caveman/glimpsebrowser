...
"""Test the glimpseproc fixture used for tests."""
...
from end2end.fixtures import glimpseprocess, testprocess
...
@pytest.fixture
def request_mock(glimpseproc, monkeypatch, server):
    ...
    monkeypatch.setattr(glimpseproc, 'request', fake_request)
    return fake_request
...
def test_glimpseproc_error_message(qtbot, glimpseproc, cmd, request_mock):
    """Make sure the test fails with an unexpected error message."""
    with qtbot.waitSignal(glimpseproc.got_error):
        glimpseproc.send_cmd(cmd)
    ...
    with pytest.raises(pytest.fail.Exception):
        glimpseproc.after_test()


def test_glimpseproc_error_message_did_fail(qtbot, glimpseproc, request_mock):
    """Make sure the test does not fail on teardown if the main test failed."""
    request_mock.node.rep_call.failed = True
    with qtbot.waitSignal(glimpseproc.got_error):
        glimpseproc.send_cmd(':message-error test')
    # Usually we wouldn't call this from inside a test, but here we force the
    # error to occur during the test rather than at teardown time.
    glimpseproc.after_test()


def test_glimpseproc_skip_via_js(qtbot, glimpseproc):
    with pytest.raises(pytest.skip.Exception, match='test'):
        glimpseproc.send_cmd(':jseval console.log("[SKIP] test");')
        glimpseproc.wait_for_js('[SKIP] test')
...
def test_glimpseproc_skip_and_wait_for(qtbot, glimpseproc):
    """This test will skip *again* during teardown, but we don't care."""
    with pytest.raises(pytest.skip.Exception):
        glimpseproc.send_cmd(':jseval console.log("[SKIP] foo");')
        glimpseproc.wait_for_js("[SKIP] foo")
        glimpseproc.wait_for(message='This will not match')


def test_qt_log_ignore(qtbot, glimpseproc):
    """Make sure the test passes when logging a qt_log_ignore message."""
    with qtbot.waitSignal(glimpseproc.got_error):
        glimpseproc.send_cmd(':message-error "SpellCheck: test"')


def test_glimpseprocess_quitting(qtbot, glimpseproc_process):
    """When glimpsebrowser quits, after_test should fail."""
    with qtbot.waitSignal(glimpseproc_process.proc.finished, timeout=15000):
        glimpseproc_process.send_cmd(':quit')
...
def test_log_line_parse(pytestconfig, data, attrs):
    line = glimpseprocess.LogLine(pytestconfig, data)
...
def test_log_line_formatted(pytestconfig,
    ...
    record = glimpseprocess.LogLine(pytestconfig, line)
...
def test_log_line_no_match(pytestconfig):
    with pytest.raises(testprocess.InvalidLine):
        glimpseprocess.LogLine(pytestconfig, "Hello World!")


class TestClickElementByText:

    @pytest.fixture(autouse=True)
    def open_page(self, glimpseproc):
        glimpseproc.open_path('data/click_element.html')

    def test_click_element(self, glimpseproc):
        glimpseproc.click_element_by_text('Test Element')
        glimpseproc.wait_for_js('click_element clicked')

    def test_click_special_chars(self, glimpseproc):
        glimpseproc.click_element_by_text('"Don\'t", he shouted')
        glimpseproc.wait_for_js('click_element special chars')

    def test_duplicate(self, glimpseproc):
        with pytest.raises(ValueError, match='not unique'):
            glimpseproc.click_element_by_text('Duplicate')

    def test_nonexistent(self, glimpseproc):
        with pytest.raises(ValueError, match='No element'):
            glimpseproc.click_element_by_text('no element exists with this text')
...
def test_xpath_escape(string, expected):
    assert glimpseprocess._xpath_escape(string) == expected
...
def test_set(glimpseproc, value):
    glimpseproc.set_setting('content.default_encoding', value)
    read_back = glimpseproc.get_setting('content.default_encoding')
...
def test_is_ignored_chromium_message(message, ignored):
    assert glimpseprocess.is_ignored_chromium_message(message) == ignored
