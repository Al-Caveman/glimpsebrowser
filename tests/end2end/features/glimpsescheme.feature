...
Feature: Special glimpse:// pages
    ...
    Scenario: :help without topic
        ...
        And I run :tab-only
        And I run :help
        And I wait until glimpse://help/index.html is loaded
        Then the following tabs should be open:
            - glimpse://help/index.html (active)
    ...
    Scenario: :help with command
        ...
        And I run :tab-only
        And I run :help :back
        And I wait until glimpse://help/commands.html#back is loaded
        Then the following tabs should be open:
            - glimpse://help/commands.html#back (active)
    ...
    Scenario: :help with setting
        ...
        And I wait until glimpse://help/settings.html#editor.command is loaded
        Then the following tabs should be open:
            - glimpse://help/settings.html#editor.command (active)
    ...
    # https://github.com/glimpsebrowser/glimpsebrowser/issues/2513
    Scenario: Opening link with glimpse:help
        When the documentation is up to date
        And I run :tab-only
        And I open glimpse:help without waiting
        And I wait for "Changing title for idx 0 to 'glimpsebrowser help'" in the log
        And I hint with args "links normal" and follow a
        Then glimpse://help/quickstart.html should be loaded

    Scenario: Opening a link with glimpse://help
        When the documentation is up to date
        And I run :tab-only
        And I open glimpse://help without waiting
        And I wait until glimpse://help/ is loaded
        And I hint with args "links normal" and follow a
        Then glimpse://help/quickstart.html should be loaded

    Scenario: Opening a link with glimpse://help/index.html/..
        When the documentation is up to date
        And I open glimpse://help/index.html/.. without waiting
    ...
    @qtwebengine_skip
    Scenario: Opening a link with glimpse://help/img/ (QtWebKit)
        When the documentation is up to date
        And I open glimpse://help/img/ without waiting
        Then "*Error while * glimpse://*" should be logged
        And "*Is a directory*" should be logged
        And "* url='glimpse://help/img'* LoadStatus.error" should be logged

    @qtwebkit_skip
    Scenario: Opening a link with glimpse://help/img/ (QtWebEngine)
        When the documentation is up to date
        And I open glimpse://help/img/ without waiting
        Then "*Error while * glimpse://*" should be logged
        And "* url='glimpse://help/img'* LoadStatus.error" should be logged
    ...
    @skip
    Scenario: Focusing input fields in glimpse://settings and entering valid value
        When I set search.ignore_case to never
        And I open glimpse://settings
    ...
    # Too flaky...
    @skip
    Scenario: Focusing input fields in glimpse://settings and entering invalid value
        When I open glimpse://settings
        # scroll to the right - the table does not fit in the default screen
        And I run :scroll-to-perc -x 100
        And I run :jseval document.getElementById('input-search.ignore_case').value = ''
        And I run :click-element id input-search.ignore_case
    ...
    Scenario: glimpse://settings CSRF via img
        When I open data/misc/glimpsescheme_csrf.html
        And I run :click-element id via-img
        Then the img request should be blocked

    Scenario: glimpse://settings CSRF via link
        When I open data/misc/glimpsescheme_csrf.html
        And I run :click-element id via-link
        Then the link request should be blocked

    Scenario: glimpse://settings CSRF via redirect
        When I open data/misc/glimpsescheme_csrf.html
        And I run :click-element id via-redirect
        Then the redirect request should be blocked

    Scenario: glimpse://settings CSRF via form
        When I open data/misc/glimpsescheme_csrf.html
        And I run :click-element id via-form
        Then the form request should be blocked

    @qtwebkit_skip
    Scenario: glimpse://settings CSRF token (webengine)
        When I open glimpse://settings
        And I run :jseval const xhr = new XMLHttpRequest(); xhr.open("GET", "glimpse://settings/set"); xhr.send()
        Then "RequestDeniedError while handling glimpse://* URL" should be logged
        And the error "Invalid CSRF token for glimpse://settings!" should be shown
    ...
    Scenario: Downloading a pdf via pdf.js button (issue 1214)
        ...
        And I open data/misc/test.pdf without waiting
        And I wait for "[glimpse://pdfjs/*] PDF * (PDF.js: *)" in the log
        And I run :jseval document.getElementById("download").click()
        And I wait for "Asking question <glimpsebrowser.utils.usertypes.Question default=* mode=<PromptMode.download: 5> text=* title='Save file to:'>, *" in the log
    ...
    Scenario: Running :pyeval
        ...
        And I wait until glimpse://pyeval/ is loaded
    ...
    Scenario: :messages without level
        ...
        And I run :messages
        Then glimpse://log/?level=info should be loaded
    ...
    Scenario: Showing messages of type 'warning' or greater
        ...
        Then glimpse://log/?level=warning should be loaded
        And the error "the-error-message" should be shown
        And the warning "the-warning-message" should be shown
        And the page should contain the plaintext "the-error-message"
        And the page should contain the plaintext "the-warning-message"
    ...
    Scenario: Using glimpse://log directly
        When I open glimpse://log without waiting
        # With Qt 5.9, we don't get a loaded message?
        And I wait for "Changing title for idx * to 'log'" in the log
        Then no crash should happen

    Scenario: Using glimpse://plainlog directly
        When I open glimpse://plainlog
        Then no crash should happen

    # :version

    Scenario: Open glimpse://version
        When I open glimpse://version
        Then the page should contain the plaintext "Version info"

    # glimpse://gpl

    Scenario: Open glimpse://gpl
        When I open glimpse://gpl
