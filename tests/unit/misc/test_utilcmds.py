...
"""Tests for glimpsebrowser.misc.utilcmds."""
...
def test_repeat_command_initial(mocker, mode_manager):
    ...
    nothing to repeat.
    """
    objreg_mock = mocker.patch('glimpsebrowser.misc.utilcmds.objreg')
...
def test_debug_log_level(mocker):
    ...
    formatter_mock = mocker.patch(
        'glimpsebrowser.misc.utilcmds.log.change_console_formatter')
    handler_mock = mocker.patch(
        'glimpsebrowser.misc.utilcmds.log.console_handler')
...
def test_window_only(mocker, monkeypatch):
    ...
    winreg_mock = mocker.patch('glimpsebrowser.misc.utilcmds.objreg')
    winreg_mock.window_registry = test_windows
    sip_mock = mocker.patch('glimpsebrowser.misc.utilcmds.sip')
...
def test_version(tabbed_browser, qapp):
    ...
    assert tabbed_browser.loaded_url == QUrl('glimpse://version/')
