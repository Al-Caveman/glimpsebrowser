...
"""Initialization of glimpsebrowser and application-wide things.

The run() function will get called once early initialization (in
glimpsebrowser.py/earlyinit.py) is done. See the glimpsebrowser.py docstring for
...
import glimpsebrowser.resources
from glimpsebrowser.completion.models import miscmodels
from glimpsebrowser.commands import runners
from glimpsebrowser.api import cmdutils
from glimpsebrowser.config import config, websettings, configfiles, configinit
from glimpsebrowser.browser import (urlmarks, history, browsertab,
                                 qtnetworkdownloads, downloads, greasemonkey)
from glimpsebrowser.browser.network import proxy
from glimpsebrowser.browser.webkit import cookies, cache
from glimpsebrowser.browser.webkit.network import networkmanager
from glimpsebrowser.extensions import loader
from glimpsebrowser.keyinput import macros
from glimpsebrowser.mainwindow import mainwindow, prompt
from glimpsebrowser.misc import (readline, ipc, savemanager, sessions,
                              crashsignal, earlyinit, sql, cmdhistory,
                              backendproblem, objects)
from glimpsebrowser.utils import (log, version, message, utils, urlutils, objreg,
                               usertypes, standarddir, error, qtutils)
# pylint: disable=unused-import
# We import those to run the cmdutils.register decorators.
from glimpsebrowser.mainwindow.statusbar import command
...
def _init_icon():
    """Initialize the icon of glimpsebrowser."""
    icon = QIcon()
    fallback_icon = QIcon()
    for size in [16, 24, 32, 48, 64, 96, 128, 256, 512]:
        filename = ':/icons/glimpsebrowser-{size}x{size}.png'.format(size=size)
        pixmap = QPixmap(filename)
        if pixmap.isNull():
            log.init.warning("Failed to load {}".format(filename))
        else:
            fallback_icon.addPixmap(pixmap)
    icon = QIcon.fromTheme('glimpsebrowser', fallback_icon)
    if icon.isNull():
...
class Quitter:
    ...
    def _compile_modules(self):
        ...
        if os.path.basename(sys.argv[0]) == 'glimpsebrowser':
            # Launched via launcher script
            return
        elif hasattr(sys, 'frozen'):
            return
        else:
            path = os.path.abspath(os.path.dirname(glimpsebrowser.__file__))
    ...
    def _get_restart_args(self, pages=(), session=None, override_args=None):
        """Get the current working directory and args to relaunch glimpsebrowser.
        ...
        if os.path.basename(sys.argv[0]) == 'glimpsebrowser':
            ...
            args = [sys.executable, '-m', 'glimpsebrowser']
            cwd = os.path.join(
                os.path.abspath(os.path.dirname(glimpsebrowser.__file__)), '..')
            if not os.path.isdir(cwd):
                # Probably running from a python egg. Let's fallback to
                # cwd=None and see if that works out.
                # See https://github.com/glimpsebrowser/glimpsebrowser/issues/323
                cwd = None
    ...
    def restart(self, pages=(), session=None, override_args=None):
        """Inner logic to restart glimpsebrowser.
