...
glimpsebrowser's initialization process roughly looks like this:
...
try:
    from glimpsebrowser.misc.checkpyver import check_python_version
except ImportError:
    try:
        # python2
        ...
        sys.stderr.write("Please don't run this script directly, do something "
                         "like   python3 -m glimpsebrowser   instead.\n")
        sys.stderr.flush()
        sys.exit(100)
check_python_version()

import argparse  # pylint: disable=wrong-import-order
from glimpsebrowser.misc import earlyinit


def get_argparser():
    """Get the argparse parser."""
    parser = argparse.ArgumentParser(prog='glimpsebrowser',
                                     description=glimpsebrowser.__description__)
    ...
    parser.add_argument('--target', choices=['auto', 'tab', 'tab-bg',
                        ...
                        help="How URLs should be opened if there is already a "
                             "glimpsebrowser instance running.")
...
def main():
    ...
    from glimpsebrowser import app
    return app.run(args)
