(function() {
    const _glimpse_script_id = "__gm_{{ scriptName }}";
    ...
    function GM_setValue(key, value) {
        ...
        localStorage.setItem(_glimpse_script_id + key, value);
    }

    function GM_getValue(key, default_) {
        checkKey(key, "GM_getValue");
        return localStorage.getItem(_glimpse_script_id + key) || default_;
    }

    function GM_deleteValue(key) {
        checkKey(key, "GM_deleteValue");
        localStorage.removeItem(_glimpse_script_id + key);
    }

    function GM_listValues() {
        const keys = [];
        for (let i = 0; i < localStorage.length; i++) {
            if (localStorage.key(i).startsWith(_glimpse_script_id)) {
                keys.push(localStorage.key(i).slice(_glimpse_script_id.length));
            }
        }
        return keys;
    }
    ...
    {% if use_proxy %}
      ...
      const glimpse_gm_window_shadow = {};  // stores local changes to window
      const glimpse_gm_windowProxyHandler = {
        get: function(target, prop) {
          if (prop in glimpse_gm_window_shadow)
            return glimpse_gm_window_shadow[prop];
          if (prop in target) {
        ...
        set: function(target, prop, val) {
          return glimpse_gm_window_shadow[prop] = val;
        },
        has: function(target, key) {
          return key in glimpse_gm_window_shadow || key in target;
        }
      };
      const glimpse_gm_window_proxy = new Proxy(
        unsafeWindow, glimpse_gm_windowProxyHandler);

      with (glimpse_gm_window_proxy) {
        // We can't return `this` or `glimpse_gm_window_proxy` from
        // `glimpse_gm_window_proxy.get('window')` because the Proxy implementation
        // does typechecking on read-only things. So we have to shadow `window`
        // more conventionally here.
        const window = glimpse_gm_window_proxy;
