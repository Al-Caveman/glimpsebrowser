...
from glimpsebrowser.utils import log, objreg, usertypes, message, debug, utils
from glimpsebrowser.commands import runners
from glimpsebrowser.api import cmdutils
from glimpsebrowser.misc import consolewidget, debugcachestats
from glimpsebrowser.utils.version import pastebin_version
...
def debug_pyeval(s, file=False, quiet=False):
    ...
    glimpsescheme.pyeval_output = out
    if quiet:
        log.misc.debug("pyeval output: {}".format(out))
    else:
        tabbed_browser = objreg.get('tabbed-browser', scope='window',
                                    window='last-focused')
        tabbed_browser.load_url(QUrl('glimpse://pyeval'), newtab=True)
...
def version(win_id, paste=False):
    ...
    tabbed_browser = objreg.get('tabbed-browser', scope='window',
                                window=win_id)
    tabbed_browser.load_url(QUrl('glimpse://version/'), newtab=True)
