...
"""Check if glimpsebrowser is run with the correct python version.
...
def check_python_version():
    ...
    if sys.hexversion < 0x03050000:
        ...
        text = ("At least Python 3.5 is required to run glimpsebrowser, but " +
                "it's running with " + version_str + ".\n")
        if (Tk and  # type: ignore
                '--no-err-windows' not in sys.argv):  # pragma: no cover
            root = Tk()
            root.withdraw()
            messagebox.showerror("glimpsebrowser: Fatal error!", text)
        else:
