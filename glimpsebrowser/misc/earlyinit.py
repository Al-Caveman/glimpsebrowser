...
# NOTE: No glimpsebrowser or PyQt import should be done here, as some early
...
def _missing_str(name, *, webengine=False):
    ...
    """
    blocks = ["Fatal error: <b>{}</b> is required to run glimpsebrowser but "
    ...
    lines = ['Please search for the python3 version of {} in your '
             ...
             'https://github.com/glimpsebrowser/glimpsebrowser/blob/master/doc/install.asciidoc'
             .format(name)]
    blocks.append('<br />'.join(lines))
    if not webengine:
        lines = ['<b>If you installed a glimpsebrowser package for your '
...
def _die(message, exception=None):
    ...
    if '--no-err-windows' in sys.argv:
        ...
        msgbox = QMessageBox(QMessageBox.Critical, "glimpsebrowser: Fatal error!",
                             message)
    ...
    for name in ['PyQt5.QtCore', 'PyQt5.QtWidgets']:
        try:
            ...
            if tkinter and '--no-err-windows' not in sys.argv:
                root = tkinter.Tk()
                root.withdraw()
                tkinter.messagebox.showerror("glimpsebrowser: Fatal error!", text)
            else:
...
def check_qt_version():
    ...
    if qVersion().startswith('5.8.'):
        log.init.warning("Running glimpsebrowser with Qt 5.8 is untested and "
                         "unsupported!")
...
def check_optimize_flag():
    """Check whether glimpsebrowser is running with -OO."""
    from glimpsebrowser.utils import log
