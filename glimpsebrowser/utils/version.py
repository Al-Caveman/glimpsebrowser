...
from glimpsebrowser.utils import log, utils, standarddir, usertypes, message
from glimpsebrowser.misc import objects, earlyinit, sql, httpclient, pastebin
...
def version():
    ...
    lines = ["glimpsebrowser v{}".format(glimpsebrowser.__version__)]
    gitver = _git_str()
    ...
    importpath = os.path.dirname(os.path.abspath(glimpsebrowser.__file__))
...
def pastebin_version(pbclient=None):
    ...
    pbclient.paste(getpass.getuser(),
                   "glimpse version info {}".format(glimpsebrowser.__version__),
                   version(),
                   private=True)
