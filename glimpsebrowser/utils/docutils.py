...
from glimpsebrowser.utils import log, utils


def is_git_repo():
    """Check if we're running from a git repository."""
    gitfolder = os.path.join(glimpsebrowser.basedir, os.path.pardir, '.git')
...
def docs_up_to_date(path):
    ...
    if hasattr(sys, 'frozen') or not is_git_repo():
        return True
    html_path = os.path.join(glimpsebrowser.basedir, 'html', 'doc', path)
    filename = os.path.splitext(path)[0]
    asciidoc_path = os.path.join(glimpsebrowser.basedir, os.path.pardir,
