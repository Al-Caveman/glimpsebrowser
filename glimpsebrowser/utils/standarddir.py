...
from glimpsebrowser.utils import log, debug, message, utils
...
def _init_runtime(args):
    ...
    if not overridden:
        ...
        # _writable_location makes sure we have a glimpsebrowser-specific subdir.
        #
        # For TempLocation:
...
def _move_macos():
    ...
    old_config = config(auto=True)  # ~/Library/Preferences/glimpsebrowser
    new_config = config()  # ~/.glimpsebrowser
...
def _move_windows():
    """Move the whole glimpsebrowser directory from Local to Roaming AppData."""
    # %APPDATA%\Local\glimpsebrowser
    old_appdata_dir = _writable_location(QStandardPaths.DataLocation)
    # %APPDATA%\Roaming\glimpsebrowser
