...
"""Backend-independent glimpse://* code.
...
from glimpsebrowser.browser import pdfjs, downloads
from glimpsebrowser.config import config, configdata, configexc, configdiff
from glimpsebrowser.utils import (version, utils, jinja, log, message, docutils,
                               objreg, urlutils)
...
class Error(Exception):
    ...
    """Exception for generic errors on a glimpse:// page."""
...
class add_handler:  # noqa: N801,N806 pylint: disable=invalid-name
    ...
    """Decorator to register a glimpse://* URL handler.

    Attributes:
        _name: The 'foo' part of glimpse://foo
    """
...
def data_for_url(url):
    ...
    # A url like "glimpse:foo" is split as "scheme:path", not "scheme:host".
    log.misc.debug("url: {}, path: {}, host {}".format(
        url.toDisplayString(), path, host))
    if not path or not host:
        new_url = QUrl()
        new_url.setScheme('glimpse')
        # When path is absent, e.g. glimpse://help (with no trailing slash)
        if host:
            new_url.setHost(host)
        # When host is absent, e.g. glimpse:help
        else:
...
def glimpse_bookmarks(_url):
    """Handler for glimpse://bookmarks. Display all quickmarks / bookmarks."""
...
def glimpse_tabs(_url):
    """Handler for glimpse://tabs. Display information about all open tabs."""
    ...
    for win_id, window in objreg.window_registry.items():
        ...
        for tab in tabbed_browser.widgets():
            if tab.url() not in [QUrl("glimpse://tabs/"), QUrl("glimpse://tabs")]:
...
@add_handler('history')
def glimpse_history(url):
    """Handler for glimpse://history. Display and serve history."""
    if url.path() == '/data':
        try:
...
@add_handler('pyeval')
def glimpse_pyeval(_url):
    """Handler for glimpse://pyeval."""
    src = jinja.render('pre.html', title='pyeval', content=pyeval_output)
    return 'text/html', src


@add_handler('spawn-output')
def glimpse_spawn_output(_url):
    """Handler for glimpse://spawn-output."""
...
def glimpse_plainlog(url):
    """Handler for glimpse://plainlog.

    An optional query parameter specifies the minimum log level to print.
    For example, glimpse://log?level=warning prints warnings and errors.
...
def _glimpse_settings_set(url):
    """Handler for glimpse://settings/set."""
    query = QUrlQuery(url)
    option = query.queryItemValue('option', QUrl.FullyDecoded)
    value = query.queryItemValue('value', QUrl.FullyDecoded)

    # https://github.com/glimpsebrowser/glimpsebrowser/issues/727
    if option == 'content.javascript.enabled' and value == 'false':
        msg = ("Refusing to disable javascript via glimpse://settings "
...
def glimpse_bindings(_url):
    """Handler for glimpse://bindings. View keybindings."""
    bindings = {}
...
@add_handler('back')
def glimpse_back(url):
    """Handler for glimpse://back.
...
def glimpse_configdiff(url):
    """Handler for glimpse://configdiff."""
    if url.path() == '/old':
        try:
...
def glimpse_pastebin_version(_url):
...
@add_handler('pdfjs')
def glimpse_pdfjs(url):
    """Handler for glimpse://pdfjs. Return the pdf.js viewer."""
    if url.path() == '/file':
...
@add_handler('warning')
def glimpse_warning(url):
    """Handler for glimpse://warning."""
    path = url.path()
    if path == '/old-qt':
