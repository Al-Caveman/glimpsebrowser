...
from glimpsebrowser.commands import userscripts, runners
from glimpsebrowser.api import cmdutils
from glimpsebrowser.config import config, configdata
from glimpsebrowser.browser import (urlmarks, browsertab, inspector, navigate,
                                 webelem, downloads)
from glimpsebrowser.keyinput import modeman, keyutils
from glimpsebrowser.utils import (message, usertypes, log, qtutils, urlutils,
                               objreg, utils, standarddir, debug)
from glimpsebrowser.utils.usertypes import KeyMode
from glimpsebrowser.misc import editor, guiprocess, objects
from glimpsebrowser.completion.models import urlmodel, miscmodels
from glimpsebrowser.mainwindow import mainwindow
...
class CommandDispatcher:
    ...
    def scroll_page(self, x: float, y: float, *,
        ...
        if not tab.url().isValid():
            # See https://github.com/glimpsebrowser/glimpsebrowser/issues/701
            return
    ...
    def tab_prev(self, count=1):
        ...
        if self._count() == 0:
            ...
            # See https://github.com/glimpsebrowser/glimpsebrowser/issues/1448
            return
    ...
    def buffer(self, index=None, count=None):
        ...
        With neither index nor count given, open the glimpse://tabs page.
        ...
        if count is None and index is None:
            self.openurl('glimpse://tabs/', tab=True)
            return
    ...
    def spawn(self, cmdline, userscript=False, verbose=False,
            ...
            userscript: Run the command as a userscript. You can use an
                        ...
                        locations:
                            - `~/.local/share/glimpsebrowser/userscripts`
                              (or `$XDG_DATA_HOME`)
                            - `/usr/share/glimpsebrowser/userscripts`
            verbose: Show notifications when the command started/exited.
            output: Whether the output should be shown in a new tab.
            detach: Whether the command should be detached from glimpsebrowser.
        ...
        def _on_proc_finished():
            if output:
                tb = objreg.get('tabbed-browser', scope='window',
                                window='last-focused')
                tb.load_url(QUrl('glimpse://spawn-output'), newtab=True)
        ...
        if userscript:
            ...
            # https://github.com/glimpsebrowser/glimpsebrowser/issues/3327
            caret = self._current_widget().caret
            caret.selection(callback=_selection_callback)
        else:
    ...
    def show_help(self, tab=False, bg=False, window=False, topic=None):
        ...
        url = QUrl('glimpse://help/{}'.format(path))
        self._open(url, tab, bg, window)
    ...
    def messages(self, level='info', plain=False, tab=False, bg=False,
        ...
        if plain:
            url = QUrl('glimpse://plainlog?level={}'.format(level))
        else:
            url = QUrl('glimpse://log?level={}'.format(level))
        self._open(url, tab, bg, window)
    ...
    def jseval(self, js_code: str, file: bool = False, quiet: bool = False, *,
        ...
        Args:
            ...
            file: Interpret js-code as a path to a file.
                  ...
                  in glimpsebrowser's data dir, e.g.
                  `~/.local/share/glimpsebrowser/js`.
    ...
    def fake_key(self, keystring, global_=False):
        """Send a fake keypress or key string to the website or glimpsebrowser.
        ...
        Args:
            ...
            global_: If given, the keys are sent to the glimpsebrowser UI.
        """
        try:
