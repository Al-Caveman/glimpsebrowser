...
from glimpsebrowser.utils import utils, objreg, log, usertypes, message, qtutils
from glimpsebrowser.misc import objects, sql
...
class WebHistory(sql.SqlTable):
    ...
    def entries_between(self, earliest, latest):
        """Iterate non-redirect, non-glimpse entries between two timestamps.
    ...
    def entries_before(self, latest, limit, offset):
        """Iterate non-redirect, non-glimpse entries occurring before a timestamp.
    ...
    def clear(self, force=False):
        ...
        (e.g. `~/.local/share/glimpsebrowser/history` on Linux) but not cookies,
    ...
    def add_from_tab(self, url, requested_url, title):
        ...
        if any(url.scheme() in ('data', 'view-source') or
               (url.scheme(), url.host()) == ('glimpse', 'back')
