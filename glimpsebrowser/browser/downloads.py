...
from glimpsebrowser.utils import (usertypes, standarddir, utils, message, log,
                               qtutils, objreg)
...
def transform_path(path):
    ...
    if re.search(r'^[A-Z]:[^\\]', path, re.IGNORECASE):
        return None
    # Paths like COM1, ...
    # See https://github.com/glimpsebrowser/glimpsebrowser/issues/82
...
class AbstractDownloadManager(QObject):
    ...
    def _remove_item(self, download):
        ...
        if sip.isdeleted(self):
            # https://github.com/glimpsebrowser/glimpsebrowser/issues/1242
            return
        try:
            idx = self.downloads.index(download)
        except ValueError:
    ...
    def get_tmpdir(self):
        ...
        if self._tmpdir is None:
            self._tmpdir = tempfile.TemporaryDirectory(
                prefix='glimpsebrowser-downloads-')
        return self._tmpdir

    def get_tmpfile(self, suggested_name):
        """Return a temporary file in the temporary downloads directory.

        The files are kept as long as glimpsebrowser is running and automatically
