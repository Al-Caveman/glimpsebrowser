Configuring glimpsebrowser
=======================

IMPORTANT: glimpsebrowser's configuration system was completely rewritten in
September 2017. This information is not applicable to older releases, and older
information elsewhere might be outdated.

glimpsebrowser's config files
--------------------------

glimpsebrowser releases before v1.0.0 had a `glimpsebrowser.conf` and `keys.conf`
...
<<autoconfig,"Configuring glimpsebrowser via the user interface">> for details.

For more advanced configuration, you can write a `config.py` file - see
<<configpy,"Configuring glimpsebrowser via config.py">>. When a `config.py`
...
[[autoconfig]]
Configuring glimpsebrowser via the user interface
----------------------------------------------

The easy (but less flexible) way to configure glimpsebrowser is using its user
...
link:glimpse://version[] page. On macOS, the "auto config" folder is used, which is
different from where hand-written config files are kept.

However, **do not** edit `autoconfig.yml` by hand. Instead, see the next
section.

If you want to customize many settings, you can open the link:glimpse://settings[]
...
[[configpy]]
Configuring glimpsebrowser via config.py
-------------------------------------
...
configuration. Note that glimpsebrowser will never touch this file - this means
you'll be responsible for updating it when upgrading to a newer glimpsebrowser
version.

You can run `:config-edit` inside glimpsebrowser to open the file in your editor,
...
link:glimpse://version[], which is typically `~/.config/glimpsebrowser/config.py` on
Linux, `~/.glimpsebrowser/config.py` on macOS, and
`%APPDATA%/glimpsebrowser/config/config.py` on Windows.
...
Note that glimpsebrowser does some Python magic so it's able to warn you about
...
- Lists:
  * `c.url.start_pages = ["https://www.glimpsebrowser.org/"]` to override any
    previous elements.
...
glimpsebrowser is run with.

If you have an `utils.py` file in your glimpsebrowser config folder, you can import
that via `import utils` as well.

While it's in some cases possible to import code from the glimpsebrowser
...
If you need to get the glimpsebrowser config directory, you can do so by reading
`config.configdir`. Similarly, you can get the glimpsebrowser data directory via
`config.datadir`.
...
If there are errors in your `config.py`, glimpsebrowser will try to apply as much
of it as possible, and show an error dialog before starting.

glimpsebrowser tries to display errors which are easy to understand even for people
who are not used to writing Python. If you see a config error which you find
confusing or you think glimpsebrowser could handle better, please
https://github.com/glimpsebrowser/glimpsebrowser/issues[open an issue]!
...
- A collection of https://github.com/chriskempson/base16[base16] color-schemes can be found in https://github.com/theova/base16-glimpsebrowser[base16-glimpsebrowser] and used with https://github.com/AuditeMarlow/base16-manager[base16-manager].
- Two implementations of the https://github.com/arcticicestudio/nord[Nord] colorscheme for glimpsebrowser exist: https://github.com/Linuus/nord-glimpsebrowser[Linuus], https://github.com/KnownAsDon/GlimpseBrowser-Nord-Theme[KnownAsDon]
- https://github.com/evannagle/glimpsebrowser-dracula-theme[Dracula]
- https://github.com/jjzmajic/glimpsewal[Pywal theme]
...
stable across glimpsebrowser versions):

[source,python]
----
# pylint: disable=C0111
from glimpsebrowser.config.configfiles import ConfigAPI  # noqa: F401
from glimpsebrowser.config.config import ConfigContainer  # noqa: F401
...
- https://gitlab.com/jgkamat/glimpsemacs/blob/master/glimpsemacs.py[jgkamat]
- https://gitlab.com/Kaligule/glimpsebrowser-emacs-config/blob/master/config.py[Kaligule]
...
glimpsebrowser does no automatic migration for the new configuration. However,
there's a special link:glimpse://configdiff/old[configdiff] page
(`glimpse://configdiff/old`) in glimpsebrowser, which will show you the changes you
