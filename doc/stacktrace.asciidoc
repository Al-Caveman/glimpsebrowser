...
When there is a fatal crash in glimpsebrowser - most of the times a
...
----
[qt-debug]
Server = https://glimpsebrowser.org/qt-debug/$arch
----
...
----
$ git clone https://github.com/glimpsebrowser/qt-debug-pkgbuild.git
...
Then run glimpsebrowser directly inside gdb like this:

----
$ gdb $(readlink -f $(which python3)) -ex 'run -m glimpsebrowser --debug'
----

Note glimpsebrowser/gdb will take a long time to start. After you reproduce the
...
When you see the _glimpsebrowser.exe has stopped working_ window, do not click
"Close the program". Instead, open your task manager, there right-click on
`glimpsebrowser.exe` and select "Create dump file". Remember the path of the dump
...
and send it to mail@glimpsebrowser.org.
