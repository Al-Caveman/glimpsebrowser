...
- Changes to userscripts:
  * glimpse-pass: Don't run `pass` if only a username is requested.
  * glimpse-pass: Support private domains like `myrouter.local`.
...
- `~/.config/glimpsebrowser/blocked-hosts` can now also contain /etc/hosts-like
...
- Updated dependencies for Windows/macOS releases:
   ...
   - Note: This release includes Qt 5.12.4 instead of Qt 5.13.0 due to
     ...
     segfaults with Qt 5.13. After Qt 5.13.1 is released, glimpsebrowser v1.8.0
     will be released with an updated Qt.
- Completely revamped Windows installer which allows installing without admin
  permissions and allows setting glimpsebrowser as default browser.
- The desktop file `glimpsebrowser.desktop` is now renamed to
  `org.glimpsebrowser.glimpsebrowser.desktop`.
...
- Workaround for renderer process crashes with Qt 5.12.4.
  If you're unable to update, you can remove `~/.cache/glimpsebrowser` for the
  same result.
...
- Functionality like hinting on pages with an element with ID `_glimpsebrowser` (such as glimpsebrowser.org) on Qt 5.12.
...
- Various changes to the Windows and macOS builds:
  ...
  * macOS: The IPC socket path used to communicate with existing instances
    changed due to changes in Qt 5.12. Please make sure to quit glimpsebrowser
    before upgrading.
- `:q` now closes the current window instead of quitting glimpsebrowser completely
...
- The metainfo file `glimpsebrowser.appdata.xml` is now renamed to
  `org.glimpsebrowser.glimpsebrowser.appdata.xml`.
- The `glimpse-pass` userscript now understands domains in gpg filenames
...
- Combining Qt 5.12 with an older PyQt can lead to issues, so a warning is
  now shown when starting glimpsebrowser with that combination.
...
- Various fixes for Qt 5.12 issues:
  ...
  * Fixed broken `glimpse://history` page.
...
- PDF.js is now searched in the correct path (if not installed system-wide)
  instead of hardcoding `~/.local/share/glimpsebrowser`.
...
- Running glimpsebrowser with QtWebKit or Qt < 5.9 now shows a warning (only
...
- The glimpse-pass userscript now has optional OTP support.
...
- The `app_id` is now set to `glimpsebrowser` for Wayland.
...
- The Makefile shipped with glimpsebrowser now supports overriding variables
  `DATADIR` and `MANDIR`.
...
- The `glimpse://settings` page is now properly sorted on Python 3.5.
...
- `glimpse://` pages now work properly on Qt 5.11.2
...
- When no documentation has been generated, the plaintext documentation now can
  be shown for more files such as `glimpse://help/userscripts.html`.
...
- Repeated escaping of entries in `glimpse://log` when refreshing page.
...
- CVE-2018-10895: Fix CSRF issue on the glimpse://settings page, leading to
  possible arbitrary code execution. See the related GitHub issue for details:
  https://github.com/glimpsebrowser/glimpsebrowser/issues/4060
...
- The Windows installer now uninstalls the old version before installing the
  new one, fixing issues with glimpsebrowser not starting after installing v1.4.0
  over v1.3.3.
...
- New `glimpse-lastpass` userscript which uses the LastPass CLI to fill passwords.
- The Makefile now installs a `/usr/share/metainfo/glimpsebrowser.appdata.xml` file.
...
- On Qt 5.10 or newer, dictionaries are now read from the glimpsebrowser data
  directory (e.g. `~/.local/share/glimpsebrowser`) instead of `/usr/share/qt`.
...
- On Qt 5.9 or newer, certificate errors now show Chromium's detailed error
  page.
- Greasemonkey scripts now support a "@glimpse-js-world" tag to run them in a
...
- CVE-2018-1000559: An XSS vulnerability on the `glimpse://history` page allowed
  ...
  https://github.com/glimpsebrowser/glimpsebrowser/issues/4011.
...
- New userscripts:
  ...
  * `glimpse-keepass` to get passwords from KeePassX.
...
- glimpsebrowser now starts properly when the PyQt5 QOpenGLFunctions package wasn't
  found.
...
- With "tox -e mkvenv-pypi", PyQt 5.10.0 is used again instead of Qt 5.10.1,
  because of an issue with Qt 5.10.1 which causes glimpsebrowser to fail to start
  ("Could not find QtWebEngineProcess").
- Unbinding keys which were bound in older glimpsebrowser versions now doesn't
  crash anymore.
...
- New special pages:
  * `glimpse://bindings` (opened via `:bind`) which shows all keybindings.
  * `glimpse://tabs` (opened via `:buffer`) which lists all tabs.
- New settings:
...
- The `hist_importer.py` script now only imports URL schemes glimpsebrowser can
  handle.
...
- HTML/JS resource files are now read into RAM on start to avoid crashes when
  changing glimpsebrowser versions while it's open.
...
- `QUTE_SELECTED_HTML` is now not set for userscripts anymore except when called
  via hints.
- The `glimpsebrowser_viewsource` userscript has been removed as
...
- There's now a `misc/Makefile` file in releases, which should help
  distributions which package glimpsebrowser, as they can run something like
...
- New `{line}` and `{column}` replacements for `editor.command` to position the
  cursor correctly.
- New `glimpse-pass` userscript as alternative to `password_fill` which allows
...
- The `asciidoc2html.py` script now uses Pygments (which is already a dependency
  of glimpsebrowser) instead of `source-highlight` for syntax highlighting.
...
- `:config-write-py` with a relative path now puts the file into the config
  directory.
- The `glimpse://version` page now also shows the uptime of glimpsebrowser.
- glimpsebrowser now prompts to create a non-existing directory when starting a
  download.
- `:jseval --file` now searches relative paths in a `js/` subdir in
  glimpsebrowser's data dir, e.g. `~/.local/share/glimpsebrowser/js`.
...
- The system's default encoding (rather than UTF-8) is now used to decode
  subprocess output.
- glimpsebrowser now ensures it's focused again after an external editor is closed.
...
- Fix crash with unknown file types loaded via `glimpse://help`.
- Scrolling performance improvements.
- Sites like `glimpse://help` now redirect to `glimpse://help/` to make sure links
  work properly.
...
- Worked around a crash with PyQt 5.9.1 compiled against Qt < 5.9.1 when using
  `:yank` or `glimpse://` URLs.
- Fixed crash when opening `glimpse://help/img`.
- Fixed `gU` (`:navigate up`) on `glimpse://help` and webservers not handling `..`
  in a URL.
...
- The `glimpse://gpl` page now works correctly again.
...
- Completely rewritten configuration system which ignores the old config file.
  See link:glimpse://help/configuring.html[] for details.
- Various documentation files got moved to the doc/ subfolder;
 `glimpsebrowser.desktop` got moved to misc/.
...
- Up/Down now navigates through the command history instead of selecting
  ...
  https://github.com/glimpsebrowser/glimpsebrowser/blob/master/doc/help/configuring.asciidoc#migrating-older-configurations[restore the old behavior].
...
- New `backend` setting to select the backend to use.
  ...
  unnecessary.
- glimpsebrowser can now be set as the default browser on macOS.
- New config commands:
...
- New `:version` command which opens `glimpse://version`.
...
- Upgrading glimpsebrowser with a version older than v0.4.0 still running now won't
...
- Improved `glimpse://history` page (with lazy loading)
...
- stdin is now closed immediately for processes spawned from glimpsebrowser.
...
- `:view-source`, `:tab-clone` and `:navigate --tab` now don't open the tab as
  ...
  active tab.
- `glimpse:*` pages now use `glimpse://*` instead (e.g. `glimpse://version` instead of
  `glimpse:version`), but the old versions are automatically redirected.
...
- Version checks when starting glimpsebrowser now also take the Qt version PyQt was compiled against into account
...
- Various new functionality with the QtWebEngine backend:
    ...
    * `:download --mhtml`
- New `glimpse:history` URL and `:history` command to show the browsing history
...
- glimpsebrowser now automatically uses QtWebEngine if QtWebKit is unavailable
...
- *New dependency:* glimpsebrowser now depends on the Qt QML module, which is
...
- Hints are now drawn natively in Qt instead of using web elements. This has a
  few implications for users:
    ...
    * Styling hints by styling `glimpsehint` elements in `user-stylesheet` was
...
- Sending a command to an existing instance (via "glimpsebrowser :reload") now
...
- Lots of improvements to and bugfixes for the QtWebEngine backend, such as
  working hints. However, using glimpsebrowser directly from git is still advised
...
- Error/warning/info messages are now shown stacked above the statusbar.
  This also added various new settings:
    ...
    * `fonts -> messages.info`
- The `glimpse:settings` page now also shows option descriptions.
- `glimpse:version` and `glimpsebrowser --version` now show various important paths
...
- glimpsebrowser now still starts with an incorrectly configured
  `$XDG_RUNTIME_DIR`.
...
- New `glimpse:bookmarks` page which displays all bookmarks and quickmarks.
...
- glimpsebrowser got a new (slightly updated) logo
...
- The optional `colorlog` dependency got removed, as glimpsebrowser now displays
...
- Fixed glimpsebrowser not starting when `sys.stderr` is `None`
...
- Don't remove `glimpsebrowser-editor-*` temporary file if editor subprocess crashed
- Userscripts are also searched in `/usr/share/glimpsebrowser/userscripts`.
- Blocked hosts are now also read from a `blocked-hosts` file in the config dir
  (e.g. `~/.config/glimpsebrowser/blocked-hosts`).
...
- New userscripts in `misc/userscripts`:
    ...
    - `glimpsedmenu` and `dmenu_glimpsebrowser` to select URLs via dmenu
...
- `:set-cmd-text` has a new `--append` argument to append to the current
  statusbar text.
- glimpsebrowser now uses `~/.netrc` if available to authenticate via HTTP.
...
- `:spawn --userscript` and `:hint` with the `userscript` target now look up
  relative paths in `~/.local/share/glimpsebrowser/userscripts` or
...
- The `completion -> web-history-max-items` setting is now also respected for
  items added after starting glimpsebrowser.
...
- Tests (a reduced subset of them) now run correctly again when DISPLAY is not
  set.
- Fixed an issue causing glimpsebrowser to crash with Python 3.5 as soon as an ad
  was blocked.
- Fixed an issue causing glimpsebrowser to not start with more recent Python 3.4
...
- When downloading to a FIFO or special file, a confirmation is displayed as
  this might cause glimpsebrowser to hang.
...
- Fixed a bug where cookies were saved despite glimpsebrowser being started in
...
- Fixed a race-condition causing an exception when starting glimpsebrowser.
...
- New flag `-d`/`--detach` for `:spawn` to detach the spawned process so it's not closed when glimpsebrowser is.
...
- Improved startup time by reading the webpage history while glimpsebrowser is open.
...
- Don't install the scripts package when installing glimpsebrowser.
...
- Workaround for glimpsebrowser not starting when there are NUL-bytes in the history (because of a currently unknown bug).
...
- Fixed exception when starting glimpsebrowser with `:set` as argument.
...
- Added missing manpage (doc/glimpsebrowser.1.asciidoc) to archive.
...
- Better save logic for the config/state:
    * Only save files if modified (e.g. don't overwrite the config if it was edited outside of glimpsebrowser and nothing was changed in glimpsebrowser).
...
- New `ui -> hide-mouse-cursor` option to hide the mouse cursor inside glimpsebrowser.
- New argument `-s` for glimpsebrowser to set a temporary config option.
...
- New `--qt-name` argument to glimpsebrowser which is passed to Qt to set `WM_CLASS`.
...
* The Windows builds come with Qt 5.4.1 which has some https://lists.schokokeks.org/pipermail/glimpsebrowser/2015-March/000054.html[related bugfixes].
...
* Set window icon and add a glimpsebrowser.ico file for Windows.
...
* Fix crashes when entering invalid values in `glimpse:settings`.
