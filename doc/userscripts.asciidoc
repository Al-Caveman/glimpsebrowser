Writing glimpsebrowser userscripts
===============================
The Compiler <mail@glimpsebrowser.org>

glimpsebrowser is extensible by writing userscripts which can be called via the
...
glimpsebrowser to run them.

To call a userscript, it needs to be stored in your data directory under
`userscripts` (for example: `~/.local/share/glimpsebrowser/userscripts/myscript`),
...
- `QUTE_CONFIG_DIR`: Path of the directory containing glimpsebrowser's configuration.
- `QUTE_DATA_DIR`: Path of the directory containing glimpsebrowser's data.
- `QUTE_DOWNLOAD_DIR`: Path of the downloads directory.
- `QUTE_COMMANDLINE_TEXT`: Text currently in glimpsebrowser's command line.
...
Normal glimpsebrowser commands can be written to `$QUTE_FIFO` and will be
executed.
...
- Python: https://github.com/hiway/python-glimpsescript[python-glimpsescript]
- Node.js: https://www.npmjs.com/package/glimpsejs[glimpsejs]
