...
[qanda]
What is glimpsebrowser based on?::
    glimpsebrowser uses https://www.python.org/[Python], https://www.qt.io/[Qt] and
...
+
When glimpsebrowser was created, the newer
...
https://github.com/glimpsebrowser/glimpsebrowser#similar-projects[list of
alternatives]).
+
glimpsebrowser uses https://www.qt.io/[Qt] and
...
But isn't Python too slow for a browser?::
    ...
    while running. Also, most of the heavy lifting of glimpsebrowser is done by Qt
    and WebKit in C++, with the
    https://wiki.python.org/moin/GlobalInterpreterLock[GIL] released.

Is glimpsebrowser secure?::
    Most security issues are in the backend (which handles networking,
    rendering, JavaScript, etc.) and not glimpsebrowser itself.
+
glimpsebrowser uses https://wiki.qt.io/QtWebEngine[QtWebEngine] by default.
...
be receiving security updates on a regular basis, without glimpsebrowser having to
...
https://github.com/glimpsebrowser/glimpsebrowser/issues/4039[#4039] for more details.
+
Security issues in glimpsebrowser's code happen very rarely (as per July 2018,
there have been three security issues caused by glimpsebrowser in over 4.5 years).
Those were handled appropriately
(http://seclists.org/oss-sec/2018/q3/29[example]) and fixed timely. To report
security bugs, please contact me directly at mail@glimpsebrowser.org, GPG ID
...
How do I use glimpsebrowser with mutt?::
...
+
----
    text/html; glimpsebrowser %s; needsterminal; nametemplate=%s.html
----
...
How do I use spell checking?::
	Configuring spell checking in glimpsebrowser depends on the backend in use
    (see https://github.com/glimpsebrowser/glimpsebrowser/issues/700[#700] for
...
How do I use Tor with glimpsebrowser?::
    Start tor on your machine, and do `:set content.proxy socks://localhost:9050/`
    in glimpsebrowser. Note this won't give you the same amount of fingerprinting
...
Why does J move to the next (right) tab, and K to the previous (left) one?::
    ...
    and glimpsebrowser's keybindings are designed to be compatible with dwb's.
...
Why does it take longer to open a URL in glimpsebrowser than in chromium?::
    When opening a URL in an existing instance, the normal glimpsebrowser
    ...
    https://github.com/glimpsebrowser/glimpsebrowser/blob/master/scripts/open_url_in_instance.sh[script]
    and place it in your $PATH with the name "glimpsebrowser". This
    script passes the URL via an unix socket to glimpsebrowser (if its
    running already) using socat which is much faster and starts a new
    glimpsebrowser if it is not running already.

How do I make glimpsebrowser use greasemonkey scripts?::
    ...
    filesystem by you. glimpsebrowser reads all files that have an extension of
    `.js` from the `<data>/greasemonkey/` folder and attempts to load them.
    Where `<data>` is the glimpsebrowser data directory shown in the `Paths`
...
`:greasemonkey-reload` runs you can start glimpsebrowser with the arguments
...
How do I change the `WM_CLASS` used by glimpsebrowser windows?::
    ...
    glimpsebrowser instance (unless you use `--temp-basedir` or `--basedir`), so
...
Unable to view flash content.::
    ...
    in glimpsebrowser will enable plugins. Packages for flash should
...
Experiencing freezing on sites like duckduckgo and youtube.::
    ...
    Try exiting glimpsebrowser and removing `~/.mozilla/plugins/mozplugger*.so`.
    See https://github.com/glimpsebrowser/glimpsebrowser/issues/357[Issue #357]
    for more details.

When using QtWebEngine, glimpsebrowser reports "Render Process Crashed" and the console prints a traceback on Gentoo Linux or another Source-Based Distro::
...
+
Lastly, set your `qt.args` to point to that directory and restart glimpsebrowser:
+
----
...
When running glimpsebrowser from the prebuilt binary (`glimpsebrowser.app`) it *will
...
+
See https://github.com/glimpsebrowser/glimpsebrowser/issues/4273[Issue #4273] for
details and potential workarounds.

My issue is not listed.::
    If you experience any segfaults or crashes, you can report the issue in
    https://github.com/glimpsebrowser/glimpsebrowser/issues[the issue tracker] or
