Contributing to glimpsebrowser
===========================
...
This document contains guidelines for contributing to glimpsebrowser, as well as
...
* Send a mail to the mailing list at mailto:glimpsebrowser@lists.glimpsebrowser.org[]
(optionally
https://lists.schokokeks.org/mailman/listinfo.cgi/glimpsebrowser[subscribe]
first).
* Join the IRC channel irc://irc.freenode.org/#glimpsebrowser[`#glimpsebrowser`] on
...
https://github.com/glimpsebrowser/glimpsebrowser/issues[issue tracker]. Some
pointers:

* https://github.com/glimpsebrowser/glimpsebrowser/labels/easy[Issues which should
be easy to solve]
* https://github.com/glimpsebrowser/glimpsebrowser/labels/component%3A%20docs[Documentation issues which require little/no coding]

If you prefer C++ or Javascript to Python, see the relevant issues which involve
work in those languages:

* https://github.com/glimpsebrowser/glimpsebrowser/issues?q=is%3Aopen+is%3Aissue+label%3A%22language%3A+c%2B%2B%22[C++] (mostly work on Qt, the library behind glimpsebrowser)
* https://github.com/glimpsebrowser/glimpsebrowser/issues?q=is%3Aopen+is%3Aissue+label%3A%22language%3A+javascript%22[JavaScript]
...
glimpsebrowser uses http://git-scm.com/[git] for its development. You can clone
the repo like this:
...
Running glimpsebrowser
-------------------

After link:install{outfilesuffix}#tox[installing glimpsebrowser via tox], you can run
`.venv/bin/glimpsebrowser --debug --temp-basedir` to test your changes with debug
logging enabled and without affecting existing running instances.

Alternatively, you can install glimpsebrowser's dependencies system-wide and run
`python3 -m glimpsebrowser --debug --temp-basedir`.
...
glimpsebrowser uses http://tox.readthedocs.org/en/latest/[tox] to run its
...
There are some useful functions for debugging in the `glimpsebrowser.utils.debug`
module.

When starting glimpsebrowser with the `--debug` flag, you also get useful debug
...
In glimpsebrowser, the policy is usually to use the Python libraries, as they
...
`ensure_valid` in `glimpsebrowser.utils.qtutils` which should get called
on all such objects. It will raise
`glimpsebrowser.utils.qtutils.QtValueError` if the value is not valid.
+
...
numbers should be range-checked using `glimpsebrowser.qtutils.check_overflow`,
...
The object registry in `glimpsebrowser.utils.objreg` is a collection of
...
Logging is used at various places throughout the glimpsebrowser code. If you add a
new feature, you should also add some strategic debug logging.

Unlike other Python projects, glimpsebrowser doesn't use a logger per file,
instead it uses custom-named loggers.

The existing loggers are defined in `glimpsebrowser.utils.log`. If your feature
...
[width="75%",cols="25%,75%"]
|=======================================================================
|critical  |Critical issue, glimpsebrowser can't continue to run.
...
glimpsebrowser has the concept of functions which are exposed to the user as
commands.
...
`glimpsebrowser.api.cmdutils` for details.
...
- `completion`: A completion function (see `glimpsebrowser.completions.models.*`)
...
glimpsebrowser handles two different types of URLs: URLs as a string, and URLs as
...
If you want to run glimpsebrowser (and thus QtWebKit) with
...
* Clone your favourite glimpsebrowser repository.
...
With the QtWebEngine backend, glimpsebrowser supports several chrome:// urls which
...
- chrome://gpucrash/ (crashes glimpsebrowser!)
- chrome://gpuhang/ (hangs glimpsebrowser!)
...
This is mostly useful for glimpsebrowser maintainers to work around issues in Qt - if you don't understand it, don't worry, just ignore it.

The hierarchy of widgets when QtWebEngine is involved looks like this:

- glimpsebrowser has a `WebEngineTab` object, which is its abstraction over QtWebKit/QtWebEngine.
...
glimpsebrowser's coding conventions are based on
...
workaround.
* Check relevant
https://github.com/glimpsebrowser/glimpsebrowser/issues?q=is%3Aopen+is%3Aissue+label%3Aqt[glimpsebrowser
...
* Mark the milestone at https://github.com/glimpsebrowser/glimpsebrowser/milestones as closed.
* Run `./.venv/bin/python3 scripts/dev/update_version.py {major,minor,patch}`.
* Run the printed instructions accordingly.
* Update `glimpsebrowser-git` PKGBUILD if dependencies/install changed.
* Announce to glimpsebrowser and glimpsebrowser-announce mailinglist.
